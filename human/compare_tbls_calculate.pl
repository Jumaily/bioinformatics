...
#
#------------------------------------------------------------------------------
# use: perl scriptname.pl ln_number Table_name
# ------------------------------------------------------------------------ 
...
$ln = trim($ARGV[0]);
$table_comp = trim($ARGV[1]);
if(!$ln or $ln eq ""){ print "Enter LN, exiting...\n"; exit; }   
if(!$table_comp or $table_comp eq ""){ print "Enter Narrow Peak Table Name!\n"; exit; }   
...
foreach $chr (@chromo[0..$#chromo]){  
   ...
   $ir="SELECT DISTINCT chromStart FROM $table_comp WHERE (chrom LIKE 'chr$chr') ORDER BY chromStart $limit";
   $sth=$dbh->prepare($ir);
   $sth->execute();
...
# ---------------------- writing to file(s) ---------------------- #
$outputfile = "/Users/taha/Desktop/data/".$table_comp."_LN".$ln.".csv";
open OUTPUT, ">$outputfile" or die "failed to open file";
print OUTPUT "Distance From TSS(bp),LN $ln - $table_comp\n"; 
