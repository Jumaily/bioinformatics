...
# 
# parse text file and insert into DB
# ----------------------------------------------
...
$dbh = DBI -> connect ('DBI:mysql:username;host=localhost', 'username', 'password') or die "Could not connect to database: $DBI::errstr";
$dbh->do("TRUNCATE $table");
$insert = $dbh->prepare("INSERT INTO $table (bead_id,tag,chromosome,startx,endx,sequence,strand,length,paired_matches) VALUES (?,?,?,?,?,?,?,?,?)");
